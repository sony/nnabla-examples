...
def res_block(x, out_ch,name):
    with nn.parameter_scope(name):
        ...
        out = PF.convolution(out, out_ch, kernel=(3,3), stride=(1,1), name='conv1')
        out = F.instance_normalization(out, gamma=None, beta=None, channel_axis=1)
        out = PF.prelu(out)
        out = F.pad(out, (1,1,1,1), 'reflect')
        out = PF.convolution(out, out_ch, kernel=(3,3), stride=(1,1), name='conv2')
        out = F.instance_normalization(out, gamma=None, beta=None, channel_axis=1)
        out += residual
        out = PF.prelu(out)
    return out
...
def layer2_1(x):
    ...
    conv2_1 = PF.convolution(pad2_1, 128, kernel=(3,3), stride=(1,1), name = 'layer2_1.1')
    conv2_1 = F.instance_normalization(conv2_1, gamma=None, beta=None,channel_axis = 1)
    conv2_1 = PF.prelu(conv2_1,name='layer2_1.3')
    pad2_2 = F.pad(conv2_1, (1,1,1,1),'reflect')
    conv2_2 = PF.convolution(pad2_2, 64, kernel=(3,3), stride=(2,2), name = 'layer2_1.5')
    conv2_2 = F.instance_normalization(conv2_2, gamma=None, beta=None,channel_axis = 1)
    conv2_2 = PF.prelu(conv2_2,name='layer2_1.7')
    return conv2_2
    
def layer3_1(x):
    pad3_1 = F.pad(x, (1,1,1,1),'reflect')
    conv3_1 = PF.convolution(pad3_1, 128, kernel=(3,3), stride=(1,1), name = 'layer3_1.1')
    conv3_1 = F.instance_normalization(conv3_1, gamma=None, beta=None,channel_axis = 1)
    conv3_1 = PF.prelu(conv3_1,name='layer3_1.3')
    pad3_2 = F.pad(conv3_1, (1,1,1,1),'reflect')
    conv3_2 = PF.convolution(pad3_2, 64, kernel=(3,3), stride=(1,1), name = 'layer3_1.5')
    conv3_2 = F.instance_normalization(conv3_2, gamma=None, beta=None,channel_axis = 1)
    conv3_2 = PF.prelu(conv3_2,name='layer3_1.7')
    return conv3_2
    
def layer4_1(x):
    pad4_1 = F.pad(x, (1,1,1,1),'reflect')
    conv4_1 = PF.convolution(pad4_1, 256, kernel=(3,3), stride=(1,1), name = 'layer4_1.1')
    conv4_1 = F.instance_normalization(conv4_1, gamma=None, beta=None,channel_axis = 1)
    conv4_1 = PF.prelu(conv4_1,name='layer4_1.3')
    pad4_2 = F.pad(conv4_1, (1,1,1,1),'reflect')
    conv4_2 = PF.convolution(pad4_2, 64, kernel=(3,3), stride=(1,1), name = 'layer4_1.5')
    conv4_2 = F.instance_normalization(conv4_2, gamma=None, beta=None,channel_axis = 1)
    conv4_2 = PF.prelu(conv4_2,name='layer4_1.7')
    up4_1 = F.interpolate(conv4_2, scale=(2, 2),mode='nearest', align_corners=False)
    return up4_1
    
def layer5_1(x):
    pad5_1 = F.pad(x, (1,1,1,1),'reflect')
    conv5_1 = PF.convolution(pad5_1, 256, kernel=(3,3), stride=(1,1), name = 'layer5_1.1')
    conv5_1 = F.instance_normalization(conv5_1, gamma=None, beta=None,channel_axis = 1)
    conv5_1 = PF.prelu(conv5_1,name='layer5_1.3')
    up5_1 = F.interpolate(conv5_1, scale=(2, 2),mode='nearest', align_corners=False)
    pad5_2 = F.pad(up5_1, (1,1,1,1),'reflect')
    conv5_2 = PF.convolution(pad5_2, 64, kernel=(3,3), stride=(1,1), name = 'layer5_1.6')
    conv5_2 = F.instance_normalization(conv5_2, gamma=None, beta=None,channel_axis = 1)
    conv5_2 = PF.prelu(conv5_2,name='layer5_1.8')
    up5_2 = F.interpolate(conv5_2, scale=(2, 2),mode='nearest', align_corners=False)
    return up5_2
...
def nonlocal_net(B_lab_map,
    ...
    A_features = layer(F.concatenate(A_feature2_1, A_feature3_1, A_feature4_1, A_feature5_1,axis=1), feature_channel * 4)
    B_features = layer(F.concatenate(B_feature2_1, B_feature3_1, B_feature4_1, B_feature5_1,axis=1), feature_channel * 4)
    #pdb.set_trace()
    # pairwise cosine similarity
    theta = PF.convolution(A_features, inter_channels, kernel=(1,1), stride=(1,1), name = 'theta')
    theta_re = F.reshape(theta,(batch_size, inter_channels, -1))
    theta_re = theta_re - F.mean(theta_re, axis = 2, keepdims=True)  # center the feature
    theta_norm = F.norm(theta_re, p=2, axis=1, keepdims=True) + sys.float_info.epsilon
    theta_re = F.div2(theta_re, theta_norm)
    theta_permute = F.transpose(theta_re,(0, 2, 1))  # 2*(feature_height*feature_width)*256
    phi = PF.convolution(B_features, inter_channels, kernel=(1,1), stride=(1,1), name = 'phi')
    phi_re = F.reshape(phi, (batch_size, inter_channels, -1))
    phi_re = phi_re - F.mean(phi_re, axis=2, keepdims=True)  # center the feature
    phi_norm = F.norm(phi_re, p=2, axis=1, keepdims=True) + sys.float_info.epsilon
    phi_re = F.div2(phi_re, phi_norm)
    f = F.batch_matmul(theta_permute, phi_re)  # 2*(feature_height*feature_width)*(feature_height*feature_width)
    #if detach_flag:
        #f = f.detach()
    ...
    similarity_map = F.reshape(similarity_map,(batch_size, 1, feature_height, feature_width))  
    ...
    f_div_C = F.softmax(f_WTA, axis=2) # 2*1936*1936; softmax along the horizontal line (dim=-1)
    ...
    y = F.reshape(y, (batch_size, channel, feature_height, feature_width))  # 2*3*44*44
    y = F.interpolate(y, scale=(4, 4),mode='nearest', align_corners=False)
    similarity_map = F.interpolate(similarity_map, scale=(4, 4),mode='nearest', align_corners=False)
...
def warp_color(IA_l, IB_lab, features_B,feature_noise=0, temperature=0.01):
    ...
    ##with torch.no_grad():
    A_relu1_1, A_relu2_1, A_relu3_1, A_relu4_1, A_relu5_1 = vgg_net(IA_rgb_from_gray, pre_process = True, fix = True)
...
def frame_colorization(IA_lab,
    ...
    color_input = F.concatenate(IA_l, nonlocal_BA_ab, similarity_map, IA_last_lab, axis=1)
