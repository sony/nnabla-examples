...
from preprocess import centerpad, lab2rgb, uncenter_l, center_crop, rgb2lab, normalize,batch_lab2rgb_transpose, save_frames,frames2vid,lab2rgb_nnabla
...
def colorize_video(args, ref):
    ...
    for iter_num, frame_name in enumerate(filenames):
        ...
        IA_lab_large = nn.Variable.from_numpy_array(transform(np.array(frame),args.image_size)) ##
        IB_lab_large = nn.Variable.from_numpy_array(transform(np.array(frame_ref), args.image_size))
        IA_lab = F.interpolate(
            IA_lab_large, scale=(0.5,0.5))
        IB_lab = F.interpolate(
            IB_lab_large, scale=(0.5,0.5))
        IA_l = IA_lab[:, 0:1, :, :]
        if I_last_lab_predict is None:
            if args.frame_propagate:
                ...
                print(type(I_last_lab_predict))
        I_current_lab = IA_lab
        I_reference_lab = IB_lab
        I_reference_l = I_reference_lab[:, 0:1, :, :]
        I_reference_ab = I_reference_lab[:, 1:3, :, :]
        I_reference_rgb = lab2rgb_nnabla(F.concatenate(uncenter_l(I_reference_l), I_reference_ab, axis=1))
        #I_reference_rgb.forward()
        #print(np.max(I_reference_rgb.d))
        # Copy to nn Variabes
        #print("vaalue", np.max(np.concatenate((uncenter_l(I_reference_l), I_reference_ab), axis=1)))
        ...
        print(type(I_reference_rgb_nn), type(I_current_lab_nn), type(I_reference_lab_nn), type(I_last_lab_predict_nn))
        t_start = time.clock()
        ...
        I_last_lab_predict =F.concatenate(IA_l, I_current_ab_predict_nn, axis=1)
        ...
        if wls_filter_on:
            ...
            wls_filter = cv2.ximgproc.createFastGlobalSmootherFilter(
                guide_image[0, 0, :, :].d.astype(np.uint8), lambda_value, sigma_color)
            ...
            curr_bs_l[:32].d, curr_predict_filter[:32, ...])
        else:
            IA_predict_rgb = batch_lab2rgb_transpose(
                curr_bs_l[:32].d, curr_predict[:32, ...])
