...
def setup_model_and_kwargs(conf: LoadedConfig,
    ...
    if i2i_strength is not None:
        conf.diffusion.t_start = int(
            conf.diffusion.max_timesteps * (1.0 - i2i_strength))
    ...
    if conf.model.text_cond:
        ...
        # (max_len, emb_dim)
        t5_emb = np.pad(t5_emb, ((0, mtl - t5_emb.shape[0]), (0, 0)))
...
class InferenceModel(object):
    ...
    def infer(self,
        ...
        if x_init is not None:
            x_init = np.tile(x_init.reshape((1,) + x_init.shape),
                             (num_gen, ) + (1, ) * len(x_init.shape))
            ...
            with nn.auto_forward():
                x_init = model.diffusion.q_sample(
                    nn.Variable.from_numpy_array(x_init), T_var)
