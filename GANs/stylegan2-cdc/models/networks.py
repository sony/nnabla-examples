...
def mapping_network(z, outmaps=512, num_layers=8, net_scope='G_mapping/Dense'):
    ...
    for i in range(num_layers):
        with nn.parameter_scope(f'{net_scope}{i}'):
            ...
            out = F.mul_scalar(F.leaky_relu(out, alpha=0.2, inplace=False), np.sqrt(2), inplace=False)
    return out
...
def styled_conv_block(conv_input, w, noise=None, res=4, inmaps=512, outmaps=512, kernel_size=3, 
    ...
    runtime_coef_for_conv = (1/np.sqrt(np.prod([inmaps, kernel_size, kernel_size]))).astype(np.float32)

    if up:
        init_function = weight_init_fn(shape=(inmaps, outmaps, kernel_size, kernel_size), return_init=True) 
        ...
        init_function = weight_init_fn(shape=(outmaps, inmaps, kernel_size, kernel_size), return_init=True) 
        conv_weight = nn.parameter.get_parameter_or_create(name=f'G_synthesis/{res}x{res}/{namescope}/conv/W', 
    ...
    if up:
        scale = F.reshape(style, (style.shape[0], style.shape[1], 1, 1, 1), inplace=False)
    else:
        scale = F.reshape(style, (style.shape[0], 1, style.shape[1], 1, 1), inplace=False)
     
    mod_w = F.mul2(F.reshape(conv_weight, (1,)+conv_weight.shape, inplace=False), scale)
    ...
    conv_input = F.reshape(conv_input, (1, -1, conv_input.shape[2], conv_input.shape[3]), inplace=False)
    demod_w = F.reshape(demod_w, (-1, demod_w.shape[2], demod_w.shape[3], demod_w.shape[4]), inplace=False)  
    
            
    if up:
        k = [1, 3, 3, 1]
        conv_out = upsample_conv_2d(conv_input, demod_w, k, factor=2, gain=1, group=batch_size)
    else:
        conv_out = F.convolution(conv_input, demod_w, pad=(pad_size, pad_size), group=batch_size)
        conv_out = F.reshape(conv_out, (batch_size, -1, conv_out.shape[2], conv_out.shape[3]), inplace=False)
    ...
    conv_out = F.add2(conv_out, F.reshape(bias, (1, outmaps, 1, 1), inplace=False))    
    
    if act == F.leaky_relu:
        conv_out = F.mul_scalar(F.leaky_relu(conv_out, alpha=0.2, inplace=False), np.sqrt(2), inplace=False)
    else:
...
def conv_layer(conv_input, inmaps, outmaps, kernel_size, downsample=False, 
    ...
    if downsample:
        k = [1, 3, 3, 1]     
        out = downsample_2d(conv_input, k, factor=2, gain=1, kernel_size=kernel_size)
        stride = 2
        pad = 0
    else:
        stride = 1
    ...
    out = F.convolution(out, conv_weight*scale, bias=conv_bias, stride=(stride,stride), pad=(pad,pad))
...
def res_block(res_input, res, inmaps, outmaps, block_scope='res_block'):
    ...
    out = F.mul_scalar(F.add2(out, skip), 1/np.sqrt(2).astype(np.float32), inplace=False)
