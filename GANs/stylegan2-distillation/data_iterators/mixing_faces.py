...
class MixingFacesData(DataSource):
    ...
    def __init__(self, data_root, image_size, img_exts, rng=313):
        ...
        self.paths_c = sorted([os.path.join(data_root, img_path) for img_path in os.listdir(data_root) if img_exts[0] in img_path])
        self.paths_s = sorted([os.path.join(data_root, img_path) for img_path in os.listdir(data_root) if img_exts[1] in img_path])
        self.paths_m = sorted([os.path.join(data_root, img_path) for img_path in os.listdir(data_root) if img_exts[2] in img_path])
        
        assert len(self.paths_c) > 0 and len(self.paths_s) > 0 and len(self.paths_m), 'No images were found in {}'.format(data_root)
        assert len(self.paths_c) == len(self.paths_s) == len(self.paths_m) , 'Unequal images of two domains found {} != {} != {}'.format(len(self.paths_c), len(self.paths_s), len(self.paths_m))
