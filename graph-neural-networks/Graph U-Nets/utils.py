...
def load_cora(shuffle=True, seed=None):
    """
...
def get_mask(labels_per_class, num_valid, num_test, num_nodes, num_classes, labels, seed=None):
    '''
...
def normalize_adj(G):
    ...
    A_tilda = A + 2 * np.eye(A.shape[0]).astype(np.float)  # A + 2I
