        ...
        for epoch in range(self.total_epochs):
            for i in range(self.max_iter):
                ...
                if print_freq and (i % print_freq == 0):
                    print(f'Training epoch {epoch}: [{i+1}|{self.max_iter}], Loss : {self.loss.d}',
                          flush=True)
            if model_save:
                ...
                if acc > self.acc:
                    ...
                    accuracy, clf_thresh = \
                        utils.get_best_classification_thershold(val_scores,
                                                                val_targets[:, 0])
                    val_pred = np.where(val_scores > clf_thresh, 1, 0)
