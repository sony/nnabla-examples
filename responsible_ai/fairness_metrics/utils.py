...
def display_log(log_dict):
    ...
    confusion_matrix_log = \
        ...
        "* The model predicted and gave favourable outcomes to " + str(log_dict['total_approvals']) + "" \
                                                                                                      " people. Among these individuals, " + str(
            log_dict['privileged_approvals']) + " belonged to " \
                                                "the privileged group and " + str(
            log_dict['unprivileged_approvals']) + " belonged to the unprivileged group.\n" \
                                                  "* Positive rate (PR) for privileged group is " + str(
            log_dict['privileged_PR']) + ", " \
                                         "and the PR for unprivileged group is " + str(
            log_dict['unprivileged_PR']) + " . " \
                                           "PR = TP /(TP+FP+TN+FN).\n" \
                                           "* True positive rate (TPR) for the privileged group is " + str(
            log_dict['privileged_TPR']) + ", " \
                                          "and TPR for the  unprivileged group is " + str(
            log_dict['unprivileged_TPR']) + " . " \
                                            "For the privileged group, the TPR = TP /(TP+FN).\n" \
                                            "* False positive rate (FPR) for the privileged group is " + str(
            log_dict['privileged_FPR']) + "," \
                                          " and the FPR for the unprivileged group is " + str(
            log_dict['unprivileged_FPR']) + " ." \
    ...
    demographic_parity_log = \
        ...
        "unprivileged and privileged group is " + str(log_dict['DPD']) + " , " \
                                                                         "which means the " + str(
            ["privileged group gets " + str(round(abs(log_dict['DPD'] * 100), 3)) + ""
                                                                                    "% more positive outcome" if
             log_dict['DPD'] < 0 else "unprivileged group"
                                      " gets " + str(round(abs(log_dict['DPD'] * 100), 3)) + "% more positive outcome"][
                0]) + ".\n" \
    ...
    # disparate impact
    disparate_impact_header = \
        "##### Disparate Impact: \n" \
        "* As discussed in the Disparate Impact section, model is fair" \
        " if all segments of the protected class receive equal proportion/rate of positive outcomes." \
        " We refer to this proportion as `Positive Rate` of the model."
    display(Markdown(demographic_parity_header))
    plot_disparate_impact(log_dict['DI'], "Disparate Impact")
    disparate_impact_log = \
        "* As seen in the above diagram , the ratio of rate of favorable outcome for the" \
        " unprivileged group to that of the privileged group is " + str(log_dict['DI']) + "\n" \
                                                                                          "* Disparate Impact (DI) must be close to One for the classifier to be fair.\n" \
                                                                                          "* Model fairness is " + str(
            log_dict['DI']) + ", which means " \
        + str(["the model treats individuals from the privileged and "
               "unprivileged group fairly, and satisfies the disparate impact fairness criterion."
               if (log_dict['DI'] == 1) else
               "the model is not satisfied Disparate Impact fairness criterion ."
               " To improve model fairness , we need to mitigate (minimize) the bias."][0])

    display(Markdown(disparate_impact_log))
    ...
    equal_opportunity_log = \
        ...
        "unprivileged and privileged group is " + str(log_dict['EOD']) + ", " \
                                                                         "which means the " + str(
            ["privileged group gets " + str(abs(log_dict['EOD'] * 100)) +
        ...
        + str(["the model treats individuals from the privileged and "
               ...
               if (-0.10 < log_dict['EOD'] < 0.10) else ""
                                                        ...
                                                        " To improve model fairness , we need to mitigate (minimize) the bias."][
                  0])
...
def load_adult_data():
    ...
    try:
        ...
        input_data = (pd.read_csv(file_path, names=column_names,
                                  na_values="?", sep=r'\s*,\s*', engine='python').loc[
            lambda df: df['race'].isin(['White', 'Black'])]).dropna()
    except Exception as e:
        print("Error : ", e)
        sys.exit(1)
...
def plot_disparate_impact(fairness, metric="None"):
    """
    graphical visualization of disparate impact fairness
    ...
    plt.text(0, fairness + 0.01, str(round(fairness, 3)), fontweight='bold',
