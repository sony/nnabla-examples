...
def vision_transformer(x, input_res, patch_size, v_width, v_layers, v_heads, embed_dim):
    ...
    with nn.parameter_scope("visual"):
        ...
        x = F.convolution(x, con1_w, bias=None, stride=(patch_size, patch_size)) # shape = [*, width, grid, grid]

        x = F.reshape(x, (x.shape[0], x.shape[1], -1)) # shape = [*, width, grid ** 2]
        ...
        x = F.concatenate(class_embed + zeros, x, axis=1) # shape = [*, grid ** 2 + 1, width]
...
def encode_image(x):
    ...
    if 'visual/proj' in param_dict:
        ...
        vision_layers = len([k for k in param_dict.keys() if k.startswith('visual/') and k.endswith('/attn/in_proj_W')])
...
def encode_text(text):
    ...
    transformer_layers = len(set(k.split('/')[2] for k in param_dict.keys() if k.startswith(f'transformer/resblocks')))
...
def logits(image, text):
    ...
    per_image = F.batch_matmul(image_features, text_features).reshape((image_features.shape[0], -1))
