    ...
    def show_all_imgs(self, path='.'):
        if not self.ipynb:
            for i, v in self.imgs.items():
                cv2.imwrite(os.path.join(path, '{}.jpg'.format(i)), v)
        else:
            self.ax = None
            nImgs = len(self.imgs)
                ...
                for i in range(len(dets[cat])):
                    if dets[cat][i, 2] > center_thresh:
                        ...
                        if show_box:
                            ...
                            self.add_coco_bbox(
                                bbox, cat - 1, dets[cat][i, 2], show_txt=show_txt, img_id=img_id)
        else:
            for i in range(len(dets)):
                if dets[i, 2] > center_thresh:
                    # print('dets', dets[i])
                    cat = int(dets[i, -1])
                    cl = (self.colors[cat, 0, 0] if self.theme ==
                          'black' else 255 - self.colors[cat, 0, 0]).tolist()
        ...
        for cat in dets:
            for i in range(len(dets[cat])):
                ...
                if dets[cat][i, -1] > center_thresh:
                    bbox = dets[cat][i, 1:5]
                    self.add_coco_bbox(
                        bbox, cat - 1, dets[cat][i, -1], show_txt=show_txt, img_id=img_id)
...
def save_nnp(opt, model, extension='nnp'):
    ...
    save(
        os.path.join(opt.save_dir, f'{model_name}.{extension}'),
        runtime_contents
    )
