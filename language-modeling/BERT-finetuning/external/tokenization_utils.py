...
from external.file_utils import cached_path, is_remote_url, hf_bucket_url
...
class PreTrainedTokenizer(object):
            ...
            if key in self.SPECIAL_TOKENS_ATTRIBUTES:
                if key == 'additional_special_tokens':
                    assert isinstance(value, (list, tuple)) and all(isinstance(
                        t, str) or (six.PY2 and isinstance(t, unicode)) for t in value)
                else:
                    assert isinstance(value, str) or (
                        six.PY2 and isinstance(value, unicode))
            ...
            for file_id, file_path in vocab_files.items():
                if file_path is None:
                    ...
                    resolved_vocab_files[file_id] = cached_path(
                        file_path, cache_dir=cache_dir, force_download=force_download, proxies=proxies, resume_download=resume_download)
    ...
    def add_tokens(self, new_tokens):
        ...
        for token in new_tokens:
            assert isinstance(token, str) or (
                six.PY2 and isinstance(token, unicode))
        ...
        added_tok_encoder = dict((tok, len(self) + i)
                                 for i, tok in enumerate(to_add_tokens))
        added_tok_decoder = {v: k for k, v in added_tok_encoder.items()}
    ...
    def tokenize(self, text, **kwargs):
        ...
        def lowercase_text(t):
            ...
            escaped_special_toks = [re.escape(s_tok)
                                    for s_tok in all_special_tokens]
    ...
    def prepare_for_model(self, ids, pair_ids=None, max_length=None, add_special_tokens=True, stride=0,
        ...
        if add_special_tokens:
            ...
            token_type_ids = [0] * \
                len(ids) + ([1] * len(pair_ids) if pair else [])
        ...
        needs_to_be_padded = pad_to_max_length and (
            ...
            or
            max_length is None and len(
                encoded_inputs["input_ids"]) < self.max_len and self.max_len <= 10000
        )
        ...
        if needs_to_be_padded:
            ...
            if self.padding_side == 'right':
                if return_attention_mask:
                    encoded_inputs["attention_mask"] = [
                        1] * len(encoded_inputs["input_ids"]) + [0] * difference
                if return_token_type_ids:
                ...
                if return_attention_mask:
                    encoded_inputs["attention_mask"] = [
                        0] * difference + [1] * len(encoded_inputs["input_ids"])
                if return_token_type_ids:
                    encoded_inputs["token_type_ids"] = [
                        self.pad_token_type_id] * difference + encoded_inputs["token_type_ids"]
                if return_special_tokens_mask:
            ...
            encoded_inputs["attention_mask"] = [
                1] * len(encoded_inputs["input_ids"])
    ...
    def truncate_sequences(self, ids, pair_ids=None, num_tokens_to_remove=0, truncation_strategy='longest_first', stride=0):
        ...
        if truncation_strategy == 'longest_first':
            ...
            assert pair_ids is not None and len(
                pair_ids) > num_tokens_to_remove
