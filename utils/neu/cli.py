...
def setup_inception_score_cli(subparsers, callbacks):
    ...
    subparsers.add_parser("inception_score", parents=[parser], conflict_handler='resolve')
...
def main():
    ...
    if args.command not in callbacks:
        raise ValueError(f"The command '{args.command}' is not supported. Must be one of {callbacks.keys()}.")
