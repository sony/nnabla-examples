...
from nnabla_rl.algorithms import MunchausenIQN, MunchausenIQNConfig
...
class ExampleStateActionQuantileSolverBuilder(SolverBuilder):
    def build_solver(self, env_info, algorithm_config, **kwargs):
        config: MunchausenIQNConfig = algorithm_config
...
def train():
    ...
    # Adjust the replay_buffer_size through MunchausenIQNConfig if you do not have enough memory on your machine.
    ...
    gpu_id = 0
    config = MunchausenIQNConfig(gpu_id=gpu_id,
                                 learning_rate=learning_rate,
                                 start_timesteps=start_timesteps,
                                 max_explore_steps=max_explore_steps)
    m_iqn = MunchausenIQN(train_env,
                          config=config,
    ...
    m_iqn.train(train_env, total_iterations=total_iterations)
